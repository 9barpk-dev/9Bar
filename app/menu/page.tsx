...
export default function MenuPage() {
  return (
    <div className="brand-page mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="mt-12 space-y-12">
        {menuSections.map((section) => (
          <section key={section.title} className="space-y-6">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-2xl font-semibold tracking-[-0.03em] text-white">{section.title}</h2>
              <span className="text-sm text-[#f8efe5]/55">{section.items.length} items</span>
            </div>
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {section.items.map((coffee) => (
                <motion.article variants={item} whileHover={{ y: -7 }} transition={{ duration: 0.25 }} key={coffee.slug} className="group overflow-hidden rounded-[20px] border border-[#c8a46a]/30 bg-[linear-gradient(135deg,rgba(255,253,249,0.96),rgba(246,240,232,0.84))] shadow-[0_18px_38px_rgba(59,42,31,0.12)]">
                  <div className="relative h-60 overflow-hidden bg-[#11141d]">
                    <Image src={coffee.image} alt={coffee.name} fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" className="object-contain transition-transform duration-700 group-hover:scale-105" />
