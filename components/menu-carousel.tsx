...
const carouselCategoryOrder = ["Frappes", "Iced Latte", "Hot", "Americano", "Long Black"];
const carouselItems = [...menuItems].sort((first, second) => carouselCategoryOrder.indexOf(first.category) - carouselCategoryOrder.indexOf(second.category));
...
export function MenuCarousel() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setActiveIndex((current) => (current + 1) % carouselItems.length), 4500);
    return () => window.clearInterval(timer);
  }, []);

  const move = (direction: -1 | 1) => setActiveIndex((current) => (current + direction + carouselItems.length) % carouselItems.length);
      ...
      <div className="overflow-hidden rounded-[28px] border border-[#d2a24c]/30 bg-[#11100e]/80 shadow-[0_20px_45px_rgba(0,0,0,0.25)]">
        <div className="grid md:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="flex flex-col justify-center p-6 sm:p-10">
            <p className="eyebrow">{String(activeIndex + 1).padStart(2, "0")} / {String(carouselItems.length).padStart(2, "0")}</p>
            ...
            <label className="mt-6 block"><span className="sr-only">Choose a menu item</span><input type="range" min="0" max={carouselItems.length - 1} value={activeIndex} onChange={(event) => setActiveIndex(Number(event.target.value))} className="h-2 w-full cursor-pointer accent-[#d2a24c]" aria-label="Choose a menu item" /></label>
          </div>
        </div>
      </div>
    </section>
