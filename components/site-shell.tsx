...
export function SiteShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-[#05070b] text-[#f8efe5]">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#05070b]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 md:flex">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-sm font-medium text-[#f8efe5]/80 transition hover:text-[#d2a24c]">
                {item.label}
              </Link>
            ))}
          </nav>
          <Link href={orderLinks.phone} className="hidden items-center gap-2 rounded-full border border-[#d2a24c]/40 px-4 py-2 text-sm font-semibold text-[#f8efe5] transition hover:bg-[#d2a24c]/10 sm:flex">
            <PhoneCall size={16} />
            Call Us
          </Link>
        </div>
